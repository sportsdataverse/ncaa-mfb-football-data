...
from ncaa_mfb_data_build.config import PKG_FUNCTION, REGISTRY
...
def test_stamp_uploads_the_four_sidecars_with_clobber():
    ...
    publish._stamp("ncaa_mfb_drives", calls.append, "sportsdataverse/sportsdataverse-data")

    assert [Path(c[3]).name for c in calls] == SIDECAR_NAMES
    assert all(c[:3] == ["release", "upload", "ncaa_mfb_drives"] for c in calls)
...
def test_stamp_names_the_loader_for_the_tag():
    ...
    publish._stamp("ncaa_mfb_drives", _runner, "sportsdataverse/sportsdataverse-data")

    expected = PKG_FUNCTION["ncaa_mfb_drives"]

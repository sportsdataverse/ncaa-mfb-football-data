...
``season`` is the **ENDING** academic year throughout (2026 = fall-2025), the
same key the raw tree uses (``academic_year`` / ``mfb/datasets/{ay}/``).
"""
...
REGISTRY: dict[str, DatasetSpec] = {
    "teams": DatasetSpec("teams", "teams/parquet/{season}_div*.parquet", "team ids/names per division (11=FBS, 12=FCS)"),
    "schedule": DatasetSpec("schedule", "schedules/parquet/{season}.parquet", "schedule master: one row per team-game"),
    "rosters": DatasetSpec("rosters", "rosters/parquet/{season}.parquet", "per-team season rosters with stats.ncaa.org player ids"),
    "pbp": DatasetSpec("pbp", "datasets/{season}/pbp.parquet", "structural NCAA play-by-play"),
    "pbp_cfbfastr": DatasetSpec("pbp_cfbfastr", "datasets/{season}/pbp_cfbfastr.parquet", "cfbfastR-named play frame"),
    "team_stats": DatasetSpec("team_stats", "datasets/{season}/team_stats.parquet", "per-period team box"),
    "player_stats": DatasetSpec("player_stats", "datasets/{season}/player_stats_*.parquet", "individual box, all categories (diagonal concat; `category` from filename)"),
    "drives": DatasetSpec("drives", "datasets/{season}/drives.parquet", "drive chart"),
    "officials": DatasetSpec("officials", "datasets/{season}/officials.parquet", "officiating crews"),
    "linescore": DatasetSpec("linescore", "datasets/{season}/linescore.parquet", "linescore + game info"),
}

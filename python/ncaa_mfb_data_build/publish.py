...
from ncaa_mfb_data_build._logging import get_logger, human_size
from ncaa_mfb_data_build.config import PKG_FUNCTION, DatasetSpec
from ncaa_mfb_data_build.io import CSV_SUFFIX  # single source of the csv extension
...
def published_seasons(
    ...
    *,
    ...
    out: set[int] = set()
    prefix, suffix = f"{spec.tag}_", ".parquet"
    for n in names:
...
def publish_dataset(
    ...
    *,
    ...
    Args:
        ...
        base: Root directory containing ``mfb/{dataset}/parquet`` + ``mfb/_release_build/{dataset}``.
    ...
    Example:
        Quick start::
            ...
            publish.publish_dataset(REGISTRY["team_stats"], 2026, base="build")
    """
    run = runner or _gh
    ...
    if make_rds:
        parquet = base / _LEAGUE / spec.name / "parquet" / f"{spec.tag}_{season}.parquet"
        rds_path = base / _LEAGUE / "_release_build" / spec.name / f"{spec.tag}_{season}.rds"
        ...
        if parquet.exists() and (
            ...
            from ncaa_mfb_data_build import rds
    ...
    if not dry_run and not exists(spec.tag, repo):
        ...
        run(
            [
                ...
                spec.tag,
                "--notes",
                f"{spec.tag} (NCAA MFB dataset, Python-built).",
            ]
        )
